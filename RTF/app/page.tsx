...
const plannerLogic = `
  ...
  function exportToGoogleSheets() {
    console.log('[v0] Starting Google Sheets export...');
    
    if (typeof XLSX === 'undefined') {
      console.error('[v0] SheetJS library not loaded!');
      alert('Excel library not loaded. Please refresh the page and try again.');
      return;
    }
    
    console.log('[v0] SheetJS library loaded');
    ...
    try {
      ...
      // Overview Sheet
      const overviewData = [
        ['RETURN TO FORM - ULTIMATE EDITION'],
        ['12-Week Fitness Planner & Tracker'],
        [''],
        ['INSTRUCTIONS:'],
        ['1. Fill in your Core Metrics on the "Core Metrics" sheet'],
        ['2. Navigate to each week tab (Week 1-12) to log your workouts'],
        ['3. Track your daily RPE scores for automatic calculation'],
        ['4. Review your progress weekly and adjust your strategy'],
        ['5. Complete the Final Summary after 12 weeks'],
        [''],
        ['FEATURES:'],
        ['• 12 weeks of detailed workout logs'],
        ['• Daily RPE tracking with auto-calculation'],
        ['• Habit trackers for consistency'],
        ['• Weekly review and strategy pages'],
        ['• Progress photos and measurements'],
        ['• Final summary and next mission planning'],
        [''],
        ['PRIMARY GOAL:', document.querySelector('[data-id="main-goal-1"]')?.value || ''],
        ['SECONDARY METRIC:', document.querySelector('[data-id="secondary-metric-1"]')?.value || ''],
        ['TARGET VALUE:', document.querySelector('[data-id="secondary-metric-2"]')?.value || ''],
      ];
      ...
      const metricsData = [
        ['CORE METRICS'],
        [''],
        ['Measurement', 'Start (Week 1)', 'Final (Week 12)', 'Change'],
        ['Weight (lbs/kg)', document.querySelector('[data-id="m-weight-start"]')?.value || '', document.querySelector('[data-id="m-weight-end"]')?.value || '', '=C4-B4'],
        ['Waist (inches/cm)', document.querySelector('[data-id="m-waist-start"]')?.value || '', document.querySelector('[data-id="m-waist-end"]')?.value || '', '=C5-B5'],
        ['Arms (inches/cm)', document.querySelector('[data-id="m-arms-start"]')?.value || '', document.querySelector('[data-id="m-arms-end"]')?.value || '', '=C6-B6'],
        ['Chest (inches/cm)', document.querySelector('[data-id="m-chest-start"]')?.value || '', document.querySelector('[data-id="m-chest-end"]')?.value || '', '=C7-B7'],
        ['Shoulders (inches/cm)', document.querySelector('[data-id="m-shoulders-start"]')?.value || '', document.querySelector('[data-id="m-shoulders-end"]')?.value || '', '=C8-B8'],
        [''],
        ['PROGRESS PHOTOS'],
        ['Start Photo', 'Upload or paste image here'],
        ['End Photo', 'Upload or paste image here'],
        [''],
        ['VISION BOARD'],
        ['Inspiration 1', ''],
        ['Inspiration 2', ''],
      ...
      const rpeData = [
        ['RPE SCALE REFERENCE'],
        [''],
        ['RPE', 'Description', 'Reps Left in Tank'],
        ['10', 'Failure / Max Effort', '0'],
        ['9', 'Very Hard', '1'],
        ['8', 'Hard', '2'],
        ['7', 'Moderate', '3+'],
        ['6-', 'Easy / Warm-up', 'Many'],
        [''],
        ['DAILY RPE METRICS (1-10 Score)'],
        [''],
        ['Metric', 'Score (1-10)', 'Description'],
        ['Workout Intensity', '', 'How hard was your main training session?'],
        ['Nutrition Adherence', '', 'How closely did you stick to your diet/macros?'],
        ['Sleep Quality', '', 'Did you hit your sleep target and wake up rested?'],
        ['Hydration/Supplements', '', 'Water intake, vitamin/supplement consistency'],
        ['Mindset/Focus', '', 'Mental clarity, stress management, discipline'],
        [''],
        ['TOTAL DAILY RPE', '=SUM(B13:B17)', 'Max 50 points'],
      ];
      const wsRPE = XLSX.utils.aoa_to_sheet(rpeData);
      XLSX.utils.book_append_sheet(wb, wsRPE, 'RPE Guide');
      
      // Generate 12 weekly sheets
      for (let week = 1; week <= PLANNED_WEEKS; week++) {
        const weekData = [
          [\`WEEK \${week} LOG\`],
          [''],
          ['WEEKLY OBJECTIVES'],
          ['Primary Focus:', document.querySelector(\`[data-id="w\${week}-focus"]\`)?.value || ''],
          [''],
          ['HABIT TRACKER (Check = 1, Uncheck = 0)'],
          ['Habit/Focus', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'],
          ['Sleep Goal Reached', '', '', '', '', '', '', ''],
          ['Nutrition Compliant', '', '', '', '', '', '', ''],
          ['Hydration Target Met', '', '', '', '', '', '', ''],
          ['Mindset/Meditation', '', '', '', '', '', '', ''],
          ['Total Workouts Completed', document.querySelector(\`[data-id="w\${week}-total-workouts"]\`)?.value || ''],
          [''],
          ['DAILY WORKOUT LOGS'],
          [''],
        ];
        
        // Add daily workout logs for each day
        const days = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'];
        days.forEach((day, dayIndex) => {
          weekData.push([\`\${day.toUpperCase()}\`]);
          weekData.push(['Planned Training:', document.querySelector(\`[data-id="w\${week}d\${dayIndex+1}-planned"]\`)?.value || '']);
          weekData.push(['Exercise', 'Set 1', 'Set 2', 'Set 3', 'Set 4', 'RPE/Notes']);
          
          for (let ex = 1; ex <= 6; ex++) {
            weekData.push([
              document.querySelector(\`[data-id="w\${week}d\${dayIndex+1}e\${ex}-name"]\`)?.value || '',
              document.querySelector(\`[data-id="w\${week}d\${dayIndex+1}e\${ex}-s1"]\`)?.value || '',
              document.querySelector(\`[data-id="w\${week}d\${dayIndex+1}e\${ex}-s2"]\`)?.value || '',
              document.querySelector(\`[data-id="w\${week}d\${dayIndex+1}e\${ex}-s3"]\`)?.value || '',
              document.querySelector(\`[data-id="w\${week}d\${dayIndex+1}e\${ex}-s4"]\`)?.value || '',
              document.querySelector(\`[data-id="w\${week}d\${dayIndex+1}e\${ex}-rpe"]\`)?.value || '',
            ]);
          }
          
          weekData.push(['']);
          weekData.push(['DAILY RPE SCORES']);
          weekData.push(['Workout Intensity', document.querySelector(\`[data-id="w\${week}d\${dayIndex+1}rpe-workout"]\`)?.value || '']);
          weekData.push(['Nutrition Adherence', document.querySelector(\`[data-id="w\${week}d\${dayIndex+1}rpe-nutrition"]\`)?.value || '']);
          weekData.push(['Sleep Quality', document.querySelector(\`[data-id="w\${week}d\${dayIndex+1}rpe-sleep"]\`)?.value || '']);
          weekData.push(['Hydration/Supplements', document.querySelector(\`[data-id="w\${week}d\${dayIndex+1}rpe-hydration"]\`)?.value || '']);
          weekData.push(['Mindset/Focus', document.querySelector(\`[data-id="w\${week}d\${dayIndex+1}rpe-mindset"]\`)?.value || '']);
          weekData.push(['Total Daily RPE', document.querySelector(\`[data-id="w\${week}d\${dayIndex+1}rpe-total"]\`)?.value || '']);
          weekData.push(['Daily Notes', document.querySelector(\`[data-id="w\${week}d\${dayIndex+1}-notes"]\`)?.value || '']);
          weekData.push(['']);
        });
        
        weekData.push(['']);
        weekData.push(['WEEKLY REVIEW']);
        weekData.push(['Summary:', document.querySelector(\`[data-id="w\${week}-summary"]\`)?.value || '']);
        weekData.push(['Lessons Learned:', document.querySelector(\`[data-id="w\${week}-lessons"]\`)?.value || '']);
        weekData.push(['Next Week Focus:', document.querySelector(\`[data-id="w\${week}-next-focus"]\`)?.value || '']);
        weekData.push(['Adjustments:', document.querySelector(\`[data-id="w\${week}-adjustments"]\`)?.value || '']);
        
        const wsWeek = XLSX.utils.aoa_to_sheet(weekData);
        XLSX.utils.book_append_sheet(wb, wsWeek, \`Week \${week}\`);
      }
      ...
      const summaryData = [
        ...
        [''],
        ['FINAL REFLECTION'],
        [document.querySelector('[data-id="final-reflection"]')?.value || ''],
        [''],
        ['QUANTIFIABLE RESULTS'],
        ['Metric', 'Start', 'End'],
        ['Weight', document.querySelector('[data-id="m-weight-start-final"]')?.value || '', document.querySelector('[data-id="m-weight-end-final"]')?.value || ''],
        ['Key Lift PR', document.querySelector('[data-id="m-lift-start"]')?.value || '', document.querySelector('[data-id="m-lift-end"]')?.value || ''],
        ['Total Workouts', '', document.querySelector('[data-id="m-total-workouts-final"]')?.value || ''],
        [''],
        ['NEXT MISSION'],
        [document.querySelector('[data-id="next-mission"]')?.value || ''],
      ];
      const wsSummary = XLSX.utils.aoa_to_sheet(summaryData);
      XLSX.utils.book_append_sheet(wb, wsSummary, 'Final Summary');
      ...
      console.log('[v0] Excel blob generated, size:', blob.size, 'bytes');
      
      // Create download link
      const url = URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.download = \`Return_to_Form_Ultimate_Edition_\${PLANNED_WEEKS}Weeks.xlsx\`;
      link.style.cssText = 'position: fixed; top: 0; left: 0; z-index: 9999;';
      document.body.appendChild(link);
      ...
      console.log('[v0] Google Sheets export completed successfully!');
      alert('Excel file downloaded! Upload it to Google Drive and open with Google Sheets to convert it. File: Return_to_Form_Ultimate_Edition_' + PLANNED_WEEKS + 'Weeks.xlsx');
      
    } catch (error) {
      console.error('[v0] Google Sheets export failed:', error);
...
export default function PlannerPage() {
  ...
  return (
    <>
      ...
      />
      <Script src="https://cdn.sheetjs.com/xlsx-0.20.1/package/dist/xlsx.full.min.js" strategy="afterInteractive" />
      ...
      <div className="control-buttons">
        ...
        <button
          ...
          onClick={() => window.exportToGoogleSheets()}
